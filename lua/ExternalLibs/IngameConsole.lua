if Debug then Debug.beginFile("IngameConsole") end
--[[
...
function IngameConsole:setupTrigger()
    ...
    TriggerAddCondition(self.trigger, Condition(function()
        local chatStr = GetEventPlayerChatString()
        if string.sub(chatStr,1,1) ~= '-' then
            self:processInput(chatStr)
        end
        return false
    end)) --console will not react to entered stuff starting with '-'. This still allows to use other chat orders like "-exec".
end
...
do
    ...
    function IngameConsole.createTriggers()
        --Exec
        local cond = Condition(execCommand_Condition)
        local execTrigger = CreateTrigger()
        TriggerAddCondition(execTrigger, cond)
